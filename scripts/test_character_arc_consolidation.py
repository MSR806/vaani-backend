...
async def test_character_arc_consolidation(db: Session):
    print(f"Testing character arc consolidation for book ID {BOOK_ID}")
    print(f"Using batch size: {CHAPTER_BATCH_SIZE} chapters")
    print(
        f"Using mega batch size: {MEGA_BATCH_SIZE} batches ({CHAPTER_BATCH_SIZE * MEGA_BATCH_SIZE} chapters total)"
    )
    ...
    )  # Limit to MAX_CONCURRENT_TASKS concurrent tasks
    ...
    for i, batch in enumerate(batch_results):
        ...
        print(
            f"Batch {i+1} (Chapters {chapters[start_idx].chapter_no}-{chapters[end_idx-1].chapter_no}): {len(batch)} characters"
        )
        for char in batch:
            ...
            print(
                f"  - {char.name} (Chapters: {chapter_ranges[0] if chapter_ranges else 'Unknown'})"
            )
