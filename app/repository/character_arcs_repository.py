...
class CharacterArcsRepository(BaseRepository[CharacterArc]):
    ...
    def get_by_type_and_source_id(self, type: str, source_id: int) -> List[CharacterArc]:
        return (
            self.db.query(CharacterArc)
            .filter(CharacterArc.type == type, CharacterArc.source_id == source_id)
            .all()
        )

    def get_by_name_type_and_source_id(
        self, name: str, type: str, source_id: int
    ...
    def get_by_id(self, character_arc_id: int) -> CharacterArc:
        ...
        if not arc:
            raise CharacterArcNotFoundException(
                f"Character arc with ID {character_arc_id} not found"
            )
    ...
    def get_character_arcs_by_book_id(self, book_id: int) -> List[CharacterArc]:
        character_arcs = (
            self.db.query(CharacterArc)
            .join(Storyboard, CharacterArc.source_id == Storyboard.id)
            .filter(Storyboard.book_id == book_id, CharacterArc.type == "STORYBOARD")
            .all()
        )
