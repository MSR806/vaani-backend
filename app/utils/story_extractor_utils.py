        ...
        for name, content in matches:
            ...
            consolidated_character = CharacterArc(
                ...
                content_json=CharacterArcContentJSON(
                    chapter_range_content=[CharacterArcContent(
...
async def build_consolidated_characters(
    ...
    for group in character_groups.groups:
        ...
        logger.info(f"Consolidated character: {consolidated_character.name} | {consolidated_character.role}")
...
async def consolidate_blood_relations_for_all_characters(
    final_characters: List[CharacterArc],
    model_settings,
    client
) -> List[CharacterArc]:
    """
    ...
    for character in final_characters:
        logger.info(f"Processing blood relations for character: {character.name} | {character.role}")
        ...
        if character.content_json.chapter_range_content:
            for entry in character.content_json.chapter_range_content:
                if hasattr(entry, 'blood_relations') and entry.blood_relations and entry.blood_relations.strip() and entry.blood_relations.lower() != "none":
        ...
        logger.info(f"Consolidated blood relations for character: {character.name} | {character.role} | {consolidated_blood_relations}")
...
async def consolidate_blood_relations_text(
    character_name: str,
    blood_relations_texts: List[str],
    model_settings,
    client
) -> str:
    """
    ...
    consolidation_prompt = BLOOD_RELATIONS_CONSOLIDATION_PROMPT_TEMPLATE.format(
        ...
        blood_relations_texts=formatted_texts
    )
