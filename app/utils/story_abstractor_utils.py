...
async def abstract_blood_relations_with_llm(
    ...
    character_mappings_text = "\n".join(
        [f"{original}: {abstract}" for original, abstract in all_character_mappings.items()]
    )

    # Create the user prompt
    user_prompt = BLOOD_RELATIONS_USER_PROMPT.format(
        character_mappings=character_mappings_text, blood_relations=blood_relations
    )
...
async def parse_character_content_json(
    character_name: str, content_json_str: str
) -> List[CharacterArcContent]:
    try:
        ...
        logger.info(
            f"Parsed {len(chapter_contents)} chapter contents for character {character_name}"
        )
...
async def abstract_character_chapter_content(
    character_arc_content: CharacterArcContent,
    original_name: str,
    abstract_name: str,
    all_character_mappings: Dict[str, str],
    client,
    ...
    try:
        ...
        logger.info(
            f"Received abstraction for {original_name} ({abstract_name}), chapter range: {character_arc_content.chapter_range}"
        )
...
async def abstract_character_content_json(
    character_name: str,
    content_json: List[CharacterArcContent],
    abstract_name: str,
    all_character_mappings: Dict[str, str],
    client,
    model: str,
    ...
    logger.info(
        f"Processing {len(chapter_contents)} chapter segments for character {character_name} as {abstract_name}"
    )
    ...
    async def process_segment(idx: int, chapter_content: CharacterArcContent):
        async with semaphore:
            logger.info(
                f"Starting segment {idx+1}/{len(chapter_contents)} for character {character_name}"
            )
            ...
            logger.info(
                f"Completed segment {idx+1}/{len(chapter_contents)} for character {character_name}"
            )
            return result
    ...
    logger.info(
        f"Completed abstraction of {len(abstractions)} chapter segments for character {character_name}"
    )

    return abstractions


async def process_character_abstractions(
    character_arcs: List[CharacterArc], client, model, temperature
) -> List[Dict[str, Any]]:
    ...
    if blood_relation_tasks:
        ...
        for i, name in enumerate(br_names):
            ...
            logger.info(
                f"Finished abstracting blood relations for {name} as {abstract_name}: {all_blood_relations[i]}"
            )
    else:
