...
def get_character_arcs_content_by_chapter_id(
    character_arcs: List[CharacterArcModel], chapter_id: int, mentioned_archetypes: set = None
    ...
    for character_arc in character_arcs:
        ...
        for chapter_range_content in character_arc_json.chapter_range_content:
            if (
                chapter_id >= chapter_range_content.chapter_range[0]
                and chapter_id <= chapter_range_content.chapter_range[1]
            ):
                character_arcs_content.append(
                    (character_arc.name, chapter_range_content.content, character_arc.id)
                )
...
async def generate_character_arc_content(
    ...
    try:
        ...
        # Log success
        logger.info(
            f"Successfully generated character arc content for {character_name} - {chapter_range}"
        )

        return {"chapter_range": chapter_range, "content": generated_content, "success": True}
    except Exception as e:
        error_msg = (
            f"Error generating character arc for {character_name} - {chapter_range}: {str(e)}"
        )
...
async def generate_character_arc(
    ...
    sorted_chapter_ranges = sorted(
        character_arc_json.chapter_range_content, key=lambda x: x.chapter_range[0]
    )

    # Process each chapter range in sequence
    for idx, template_content in enumerate(sorted_chapter_ranges):
        logger.info(
            f"Starting segment {idx+1}/{len(character_arc_json.chapter_range_content)} for character {name}"
        )
        ...
        logger.info(
            f"Completed segment {idx+1}/{len(character_arc_json.chapter_range_content)} for character {name}"
        )

    logger.info(
        f"Completed sequential generation of {len(results)} chapter segments for character {name}"
    )
