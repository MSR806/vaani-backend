...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Generate character arcs and plot beats from templates")
    parser.add_argument("book_id", type=int, help="Book ID to use for templates (REQUIRED)")
    args = parser.parse_args()
    
    book_id = args.book_id
    logger.info(f"Initializing Story Generator for book ID: {book_id}")
    
    try:
        # Initialize story generator
        generator = StoryGenerator(book_id)
        await generator.initialize()
        
        # Just display summary of available templates``
        print(f"\n=== Story Generator ===")
        print(f"Working with book ID: {book_id}")
        print(f"Found {len(generator.character_arc_templates)} character arc templates and {len(generator.plot_beat_templates)} plot beat templates")
        print("Characters:")
        for arc in generator.character_arc_templates:
            print(f"- {arc['name']}")
        print("\n")
        
        # Get a single prompt for all characters
        print("\nEnter a brief description of the story or character ensemble you want to create.")
        print("Example: 'A heist story set in a cyberpunk future' or 'A group of misfits on a space adventure'")
        story_prompt = get_validated_input("Story prompt", required=True)
            
        # Generate all character arcs from the single prompt
        character_arcs = await generator.generate_characters_from_prompt(story_prompt)
                
        if character_arcs:
            print(f"\nSuccessfully generated {len(character_arcs)} character arcs:")
            for arc in character_arcs:
                print(f"Character arc generated for {arc['name']}")
                # Get file path from original response if available
                if 'file_path' in arc:
                    print(f"Saved to: {arc['file_path']}")


        # Ask about generating plot beats
        print("\nGenerate plot beats? This will create a plot outline that incorporates all character arcs.")
        generate_plot = get_yes_no_input("Generate plot beats", default=True)
        
        plot_beats_till_now = ""
        plot_summary_till_now = ""
        for template_idx, template in enumerate(generator.plot_beat_templates):
            plot_beats = await generator.generate_plot_beats_from_prompt(template["name"], story_prompt, character_arcs, plot_summary_till_now)
            plot_beats_till_now += f"## Plot Beat {template_idx + 1}\n\n{plot_beats}\n\n"
            
            # After generating each plot beat, create a summary of the entire plot so far
            if template_idx < len(generator.plot_beat_templates) - 1:  # Only summarize if there are more beats to generate
                plot_summary_till_now = await generator.summarize_plot_till_now(plot_beats_till_now)
