...
from app.models.models import Book, Chapter, Character
...
def create_character(db: Session, character: CharacterCreate):
    ...
    db_character = Character(
        name=character.name, description=character.description, book_id=character.book_id
    )
    db.add(db_character)
    db.commit()
        ...
        try:
            ...
            try:
                ...
                if not isinstance(retry_data, dict) or "characters" not in retry_data:
                    raise Exception(
                        f"Failed to generate a properly structured response after retry: {str(e)}"
                    )
                ...
                raise Exception(
                    f"Failed to generate a valid response even after retry: {str(retry_error)}"
                )
...
async def generate_character_outline(
    db: Session, character_id: int, request: CharacterOutlineRequest
):
    # Get the character
    character = db.query(Character).filter(Character.id == character_id).first()
    if not character:
...
User's Request: {request.user_prompt}""",
    ...
    try:
        client = get_openai_client()
        response = client.chat.completions.create(
            model=OPENAI_MODEL, messages=messages, temperature=0.7, max_tokens=2000
        )
        ...
        return {"name": character.name, "description": content}
