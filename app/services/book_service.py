...
async def create_book(db: Session, book: BookBase, user_id: str) -> Book:
    ...
    try:
        ...
        placeholder_url = await generate_placeholder_image(
            text=f"Title: {db_book.title}\nAuthor: {db_book.author}"
        )

        # Use the image service to store the image from the URL
        placeholder_image = await store_image_from_url(
            db=db, url=placeholder_url, name=f"placeholder_book_cover_{db_book.id}", user_id=user_id
        )
...
def get_books(db: Session) -> list[Book]:
    ...
    books = (
        db.query(
            Book, func.coalesce(chapter_count_subquery.c.chapter_count, 0).label("chapter_count")
        )
...
async def generate_chapter_outline(
    db: Session, book_id: int, request: ChapterGenerateRequest
) -> dict:
...
Please create a detailed outline for this chapter:""",
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to process the generated outline: {str(e)}"
            )
...
async def generate_chapter_content(
    ...
    # Prepare context from previous chapters
    previous_chapters_context = "\n\n".join(
        [f"Chapter {ch.chapter_no}: {ch.title}\n{ch.content}" for ch in previous_chapters]
    )
...
Please write the complete chapter:""",
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to process the generated chapter: {str(e)}"
            )
    ...
    try:
        ...
        try:
            ...
            db_image = await store_image_from_url(
                db=db, url=image_url, name=f"book_cover_{book_id}_{book_title}", user_id=user_id
            )
