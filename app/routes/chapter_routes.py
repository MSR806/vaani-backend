...
async def generate_chapter_outline(
    book_id: int, request: OutlineRequest, db: Session = Depends(get_db)
):
...
Please provide a structured outline for this chapter:""",
    ...
    try:
        ...
        try:
            ...
            for scene_idx, scene in enumerate(scenes_data["scenes"]):
                ...
                for char_idx, char_name in enumerate(scene["characters"]):
                    if not isinstance(char_name, str) or not char_name.strip():
                        raise ValueError(
                            f"Character {char_idx} in scene {scene_idx} must be a non-empty string"
                        )
                    if char_name not in available_characters:
                        raise ValueError(
                            f"Character '{char_name}' in scene {scene_idx} is not in the available characters list"
                        )
                ...
                if (
                    not isinstance(scene["content"], str)
                    or not scene["content"].strip()
                ):
            ...
            raise HTTPException(
                status_code=500, detail=f"Error processing the response: {str(e)}"
            )
...
@router.post(
    "/books/{book_id}/chapters/{chapter_id}/generate", response_model=ChapterResponse
)
